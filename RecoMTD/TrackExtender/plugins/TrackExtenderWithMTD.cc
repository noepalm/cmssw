...
namespace {
constexpr float c_cm_ns = geant_units::operators::convertMmToCm(
    CLHEP::c_light);  // [mm/ns] -> [cm/ns]
...
class MTDHitMatchingInfo {
 public:
  ...
  // Operator used to sort the hits while performing the matching step at the
  // MTD
  inline bool operator<(const MTDHitMatchingInfo& m2) const {
    // only for good matching in time use estChi2, otherwise use mostly time
    // compatibility
  ...
  inline float chi2(float timeWeight = 1.f) const {
 ...
 public:
  ...
  inline float computeTof(float mass_inv2) const {
    ...
    for (uint32_t iSeg = 0; iSeg < nSegment_; iSeg++) {
      ...
      LogTrace("TrackExtenderWithMTD")
          << " TOF Segment # " << iSeg + 1
          << " p = " << std::sqrt(segmentMom2_[iSeg]) << " tof = " << tof;
      ...
      LogTrace("TrackExtenderWithMTD")
          << "TOF Segment # " << iSeg + 1 << std::fixed << std::setw(6)
          << " tof segment = " << segmentPathOvc_[iSeg] / beta
  ...
  inline float computeSigmaTof(float mass_inv2) {
    ...
    for (uint32_t iSeg = 0; iSeg < nSegment_; iSeg++) {
      sigma = segmentPathOvc_[iSeg] * segmentSigmaMom_[iSeg] /
              (segmentMom2_[iSeg] * sqrt(segmentMom2_[iSeg] + 1 / mass_inv2) *
               mass_inv2);
  ...
  inline std::pair<float, float> getSegmentPathAndMom2(
      uint32_t iSegment) const {
    if (iSegment >= nSegment_) {
      throw cms::Exception("TrackExtenderWithMTD")
...
const TrackTofPidInfo computeTrackTofPidInfo(
    float magp2, float length, TrackSegments trs, float t_mtd, float t_mtderr,
    float t_vtx, float t_vtx_err, bool addPIDError = true,
  ...
  auto sigmadeltat = [&](const float mass_inv2) {
    ...
    switch (sigma_choice) {
      case SigmaTofCalc::kCost:
        ...
        res = tofpid.pathlength * c_inv *
              trs.segmentSigmaMom_[trs.nSegment_ - 1] /
      ...
      case SigmaTofCalc::kMixd:
        float res1 = tofpid.pathlength * c_inv *
                     trs.segmentSigmaMom_[trs.nSegment_ - 1] /
                     (magp2 * sqrt(magp2 + 1 / mass_inv2) * mass_inv2);
  ...
  tofpid.dt = tofpid.tmtd - tofpid.dt_pi -
              t_vtx;  // assume by default the pi hypothesis
...
bool getTrajectoryStateClosestToBeamLine(
    const Trajectory& traj, const reco::BeamSpot& bs,
    const Propagator* thePropagator, TrajectoryStateClosestToBeamLine& tscbl) {
  // get the state closest to the beamline
  TrajectoryStateOnSurface stateForProjectionToBeamLineOnSurface =
      traj.closestMeasurement(GlobalPoint(bs.x0(), bs.y0(), bs.z0()))
          .updatedState();
  ...
  const FreeTrajectoryState& stateForProjectionToBeamLine =
      *stateForProjectionToBeamLineOnSurface.freeState();
...
bool trackPathLength(const Trajectory& traj,
                     ...
                     const Propagator* thePropagator, float& pathlength,
                     TrackSegments& trs) {
  pathlength = 0.f;

  bool validpropagation = true;
  float oldp =
      traj.measurements().begin()->updatedState().globalMomentum().mag();
  float pathlength1 = 0.f;
  float pathlength2 = 0.f;

  // add pathlength layer by layer
  for (auto it = traj.measurements().begin();
       it != traj.measurements().end() - 1; ++it) {
    const auto& propresult = thePropagator->propagateWithPath(
        it->updatedState(), (it + 1)->updatedState().surface());
    ...
    float sigma_p =
        sqrt((it + 1)->updatedState().curvilinearError().matrix()(0, 0)) *
        (it + 1)->updatedState().globalMomentum().mag2();

    trs.addSegment(layerpathlength,
                   (it + 1)->updatedState().globalMomentum().mag2(), sigma_p);

    LogTrace("TrackExtenderWithMTD")
        << "TSOS " << std::fixed << std::setw(4) << trs.size() << " R_i "
        << std::fixed << std::setw(14)
        << it->updatedState().globalPosition().perp() << " z_i " << std::fixed
        << std::setw(14) << it->updatedState().globalPosition().z() << " R_e "
        << std::fixed << std::setw(14)
        << (it + 1)->updatedState().globalPosition().perp() << " z_e "
        << std::fixed << std::setw(14)
        << (it + 1)->updatedState().globalPosition().z() << " p " << std::fixed
        << std::setw(14) << (it + 1)->updatedState().globalMomentum().mag()
  ...
  auto const& aSurface = traj.direction() == alongMomentum
  ...
  float sigma_p = sqrt(tscblPCA.curvilinearError().matrix()(0, 0)) *
                  tscblPCA.momentum().mag2();

  trs.addSegment(pathlength2, tscblPCA.momentum().mag2(), sigma_p);

  LogTrace("TrackExtenderWithMTD")
      << "TSOS " << std::fixed << std::setw(4) << trs.size() << " R_e "
      << std::fixed << std::setw(14) << tscblPCA.position().perp() << " z_e "
      << std::fixed << std::setw(14) << tscblPCA.position().z() << " p "
      << std::fixed << std::setw(14) << tscblPCA.momentum().mag() << " dp "
      << std::fixed << std::setw(14) << tscblPCA.momentum().mag() - oldp
      << " sigma_p = " << std::fixed << std::setw(14) << sigma_p
...
bool trackPathLength(const Trajectory& traj, const reco::BeamSpot& bs,
                     const Propagator* thePropagator, float& pathlength,
                     TrackSegments& trs) {
  pathlength = 0.f;
  ...
  bool tscbl_status =
      getTrajectoryStateClosestToBeamLine(traj, bs, thePropagator, tscbl);

  if (!tscbl_status) return false;
...
class TrackExtenderWithMTDT : public edm::stream::EDProducer<> {
 public:
  ...
  void fillValueMap(edm::Event& iEvent, const H& handle,
  ...
  TransientTrackingRecHit::ConstRecHitContainer tryBTLLayers(
      const TrajectoryStateOnSurface&, const Trajectory& traj, const float,
      const float, const TrackSegments&, const MTDTrackingDetSetVector&,
      const MTDDetLayerGeometry*, const MagneticField* field,
      const Propagator* prop, const reco::BeamSpot& bs, const float vtxTime,
      const bool matchVertex, MTDHitMatchingInfo& bestHit) const;

  TransientTrackingRecHit::ConstRecHitContainer tryETLLayers(
      const TrajectoryStateOnSurface&, const Trajectory& traj, const float,
      const float, const TrackSegments&, const MTDTrackingDetSetVector&,
      const MTDDetLayerGeometry*, const MagneticField* field,
      const Propagator* prop, const reco::BeamSpot& bs, const float vtxTime,
  ...
  void fillMatchingHits(const DetLayer*, const TrajectoryStateOnSurface&,
                        const Trajectory&, const float, const float,
                        const TrackSegments&, const MTDTrackingDetSetVector&,
                        const Propagator*, const reco::BeamSpot&, const float&,
                        const bool,
  ...
  RefitDirection::GeometricalDirection checkRecHitsOrdering(
    ...
    if (!recHits.empty()) {
      GlobalPoint first =
          gtg_->idToDet(recHits.front()->geographicalId())->position();
      GlobalPoint last =
          gtg_->idToDet(recHits.back()->geographicalId())->position();
  ...
  reco::Track buildTrack(const reco::TrackRef&, const Trajectory&,
                         const Trajectory&, const reco::BeamSpot&,
                         const MagneticField* field, const Propagator* prop,
                         bool hasMTD, float& pathLength, float& tmtdOut,
                         float& sigmatmtdOut, float& tofpi, float& tofk,
                         float& tofp, float& sigmatofpi, float& sigmatofk,
...
template <class TrackCollection>
TrackExtenderWithMTDT<TrackCollection>::TrackExtenderWithMTDT(
    const ParameterSet& iConfig)
    : tracksToken_(consumes<InputCollection>(
          iConfig.getParameter<edm::InputTag>("tracksSrc"))),
      trajTrackAToken_(consumes<TrajTrackAssociationCollection>(
          iConfig.getParameter<edm::InputTag>("trjtrkAssSrc"))),
      hitsToken_(consumes<MTDTrackingDetSetVector>(
          iConfig.getParameter<edm::InputTag>("hitsSrc"))),
      ...
      transientTrackBuilder_(
          iConfig.getParameter<std::string>("TransientTrackBuilder")),
  ...
  if (useVertex_) {
    if (useSimVertex_) {
      genVtxPositionToken_ = consumes<GlobalPoint>(
          iConfig.getParameter<edm::InputTag>("genVtxPositionSrc"));
      genVtxTimeToken_ =
          consumes<float>(iConfig.getParameter<edm::InputTag>("genVtxTimeSrc"));
    } else
      vtxToken_ = consumes<VertexCollection>(
          iConfig.getParameter<edm::InputTag>("vtxSrc"));
  }

  theEstimator =
      std::make_unique<Chi2MeasurementEstimator>(estMaxChi2_, estMaxNSigma_);
  theTransformer = std::make_unique<TrackTransformer>(
      iConfig.getParameterSet("TrackTransformer"), consumesCollector());
  ...
  pathLengthOrigTrkToken_ =
      produces<edm::ValueMap<float>>("generalTrackPathLength");
  tmtdOrigTrkToken_ = produces<edm::ValueMap<float>>("generalTracktmtd");
  sigmatmtdOrigTrkToken_ =
      produces<edm::ValueMap<float>>("generalTracksigmatmtd");
  ...
  sigmatofpiOrigTrkToken_ =
      produces<edm::ValueMap<float>>("generalTrackSigmaTofPi");
  sigmatofkOrigTrkToken_ =
      produces<edm::ValueMap<float>>("generalTrackSigmaTofK");
  ...
  builderToken_ = esConsumes<TransientTrackBuilder, TransientTrackRecord>(
      edm::ESInputTag("", transientTrackBuilder_));
  hitbuilderToken_ =
      esConsumes<TransientTrackingRecHitBuilder, TransientRecHitRecord>(
          edm::ESInputTag("", mtdRecHitBuilder_));
  gtgToken_ =
      esConsumes<GlobalTrackingGeometry, GlobalTrackingGeometryRecord>();
  ...
  propToken_ = esConsumes<Propagator, TrackingComponentsRecord>(
      edm::ESInputTag("", propagator_));
...
template <class TrackCollection>
void TrackExtenderWithMTDT<TrackCollection>::fillDescriptions(
  ...
  desc.add<edm::InputTag>("genVtxPositionSrc",
                          edm::InputTag("genParticles:xyz0"));
  ...
  TrackTransformer::fillPSetDescription(
      ...
      "KFSmootherForRefitInsideOut", "PropagatorWithMaterialForMTD",
      "alongMomentum", true, "WithTrackAngle", "MuonRecHitBuilder",
      "MTDRecHitBuilder");
...
template <class TrackCollection>
template <class H, class T>
void TrackExtenderWithMTDT<TrackCollection>::fillValueMap(
    edm::Event& iEvent, const H& handle, const std::vector<T>& vec,
    const edm::EDPutToken& token) const {
...
template <class TrackCollection>
void TrackExtenderWithMTDT<TrackCollection>::produce(
  ...
  TrackingRecHitRefProd hitsRefProd =
      ev.getRefBeforePut<TrackingRecHitCollection>();
  reco::TrackExtraRefProd extrasRefProd =
      ev.getRefBeforePut<reco::TrackExtraCollection>();
  ...
  for (const auto& trjtrk : trjtrks) {
    ...
    LogTrace("TrackExtenderWithMTD")
        ...
        << " p/pT = " << track->p() << " " << track->pt()
        << " eta = " << track->eta();
    LogTrace("TrackExtenderWithMTD")
        << "TrackExtenderWithMTD: sigma_p = "
    ...
    if (useVertex_) {
      ...
      if (dz < dzCut_) trackVtxTime = vtxTime;
    }
    ...
    if (trajs.isValid()) {
      ...
      TrajectoryStateOnSurface tsos =
          builder_->build(track).outermostMeasurementState();
      TrajectoryStateClosestToBeamLine tscbl;
      bool tscbl_status =
          getTrajectoryStateClosestToBeamLine(trajs, bs, prop, tscbl);
      ...
      if (tscbl_status) {
        ...
        const auto& btlhits =
            tryBTLLayers(tsos, trajs, pmag2, pathlength0, trs0, hits,
                         geo.product(), magfield.product(), prop, bs,
                         trackVtxTime, trackVtxTime != 0.f, mBTL);
        mtdthits.insert(mtdthits.end(), btlhits.begin(), btlhits.end());

        // in the future this should include an intermediate refit before
        // propagating to the ETL for now it is ok
        const auto& etlhits =
            tryETLLayers(tsos, trajs, pmag2, pathlength0, trs0, hits,
                         geo.product(), magfield.product(), prop, bs,
                         trackVtxTime, trackVtxTime != 0.f, mETL);
    ...
    const auto& trajwithmtd = mtdthits.empty()
                                  ? std::vector<Trajectory>(1, trajs)
                                  : theTransformer->transform(ttrack, thits);
    float pMap = 0.f, betaMap = 0.f, t0Map = 0.f, sigmat0Map = -1.f,
          pathLengthMap = -1.f, tmtdMap = 0.f, sigmatmtdMap = -1.f,
          tofpiMap = 0.f, tofkMap = 0.f, tofpMap = 0.f, sigmatofpiMap = -1.f,
          sigmatofkMap = -1.f, sigmatofpMap = -1.f;
    int iMap = -1;

    for (const auto& trj : trajwithmtd) {
      const auto& thetrj = (updateTraj_ ? trj : trajs);
      float pathLength = 0.f, tmtd = 0.f, sigmatmtd = -1.f, tofpi = 0.f,
            tofk = 0.f, tofp = 0.f, sigmatofpi = -1.f, sigmatofk = -1.f,
            sigmatofp = -1.f;
      LogTrace("TrackExtenderWithMTD")
      ...
      reco::Track result = buildTrack(
          track, thetrj, trj, bs, magfield.product(), prop,
          !trajwithmtd.empty() && !mtdthits.empty(), pathLength, tmtd,
          sigmatmtd, tofpi, tofk, tofp, sigmatofpi, sigmatofk, sigmatofp);
      if (result.ndof() >= 0) {
        ...
        if (updatePattern_) {
          t2t(trj, *outhits, trajParams,
              chi2s);  // this fills the output hit collection
        } else {
          t2t(thetrj, *outhits, trajParams, chi2s);
        }
        size_t hitsend = outhits->size();
        extras->push_back(
            buildTrackExtra(trj));  // always push back the fully built extra,
                                    // update by setting in track
        ...
        t0Map = backtrack.t0();
        sigmat0Map = std::copysign(std::sqrt(std::abs(backtrack.covt0t0())),
                                   backtrack.covt0t0());
        tofpiMap = tofpi;
        tofkMap = tofk;
        tofpMap = tofp;
        ...
        npixBarrel.push_back(
            backtrack.hitPattern().numberOfValidPixelBarrelHits());
        npixEndcap.push_back(
            backtrack.hitPattern().numberOfValidPixelEndcapHits());
        LogTrace("TrackExtenderWithMTD")
            << "TrackExtenderWithMTD: tmtd " << tmtdMap << " +/- "
            << sigmatmtdMap << " t0 " << t0Map << " +/- " << sigmat0Map
            << " tof pi/K/p " << tofpiMap << "+/-"
            << fmt::format("{:0.2g}", sigmatofpiMap) << " ("
            << fmt::format("{:0.2g}", sigmatofpiMap / tofpiMap * 100) << "%) "
            << tofkMap << "+/-" << fmt::format("{:0.2g}", sigmatofkMap) << " ("
            << fmt::format("{:0.2g}", sigmatofkMap / tofkMap * 100) << "%) "
            << tofpMap << "+/-" << fmt::format("{:0.2g}", sigmatofpMap) << " ("
...
void find_hits_in_dets(const MTDTrackingDetSetVector& hits,
                       const Trajectory& traj, const DetLayer* layer,
                       const TrajectoryStateOnSurface& tsos, const float pmag2,
                       const float pathlength0, const TrackSegments& trs0,
                       const float vtxTime, const reco::BeamSpot& bs,
                       const float bsTimeSpread, const Propagator* prop,
  ...
  pair<bool, TrajectoryStateOnSurface> comp =
      layer->compatible(tsos, *prop, *estimator);
  if (comp.first) {
    const vector<DetLayer::DetWithState> compDets =
        layer->compatibleDets(tsos, *prop, *estimator);
    LogTrace("TrackExtenderWithMTD")
        << "Hit search: Compatible dets " << compDets.size();
    if (!compDets.empty()) {
      for (const auto& detWithState : compDets) {
        auto range = hits.equal_range(detWithState.first->geographicalId(),
                                      cmp_for_detset);
        if (range.first == range.second) {
          LogTrace("TrackExtenderWithMTD")
              << "Hit search: no hit in DetId "
              << detWithState.first->geographicalId().rawId();
          continue;
        }
        ...
        if (pl.second == 0.) {
          LogTrace("TrackExtenderWithMTD")
              << "Hit search: no propagation to DetId "
              << detWithState.first->geographicalId().rawId();
          continue;
        }
        ...
        for (auto detitr = range.first; detitr != range.second; ++detitr) {
          for (const auto& hit : *detitr) {
            ...
            if (!est.first) {
              LogTrace("TrackExtenderWithMTD")
                  << "Hit search: no compatible estimate in DetId "
                  << detWithState.first->geographicalId().rawId()
                  << " for hit at pos (" << std::fixed << std::setw(14)
                  << hit.globalPosition().x() << "," << std::fixed
                  << std::setw(14) << hit.globalPosition().y() << ","
                  << std::fixed << std::setw(14) << hit.globalPosition().z()
                  << ")";
              continue;
            }

            LogTrace("TrackExtenderWithMTD")
                << "Hit search: spatial compatibility DetId "
                << detWithState.first->geographicalId().rawId()
                << " TSOS dx/dy " << std::fixed << std::setw(14)
                << std::sqrt(
                       detWithState.second.localError().positionError().xx())
                << " " << std::fixed << std::setw(14)
                << std::sqrt(
                       detWithState.second.localError().positionError().yy())
                << " hit dx/dy " << std::fixed << std::setw(14)
                << std::sqrt(hit.localPositionError().xx()) << " " << std::fixed
                << std::setw(14) << std::sqrt(hit.localPositionError().yy())
                << " chi2 " << std::fixed << std::setw(14) << est.second;

            TrackTofPidInfo tof = computeTrackTofPidInfo(
                lastpmag2, std::abs(pl.second), trs0, hit.time(),
                hit.timeError(), t_vtx,
                t_vtx_err,  // put vtx error by hand for the moment
                false, TofCalc::kMixd, SigmaTofCalc::kMixd);
            MTDHitMatchingInfo mi;
            mi.hit = &hit;
            mi.estChi2 = est.second;
            mi.timeChi2 = tof.dtchi2_best;  // use the chi2 for the best
                                            // matching hypothesis
...
template <class TrackCollection>
TransientTrackingRecHit::ConstRecHitContainer
TrackExtenderWithMTDT<TrackCollection>::tryBTLLayers(
    const TrajectoryStateOnSurface& tsos, const Trajectory& traj,
    const float pmag2, const float pathlength0, const TrackSegments& trs0,
    const MTDTrackingDetSetVector& hits, const MTDDetLayerGeometry* geo,
    const MagneticField* field, const Propagator* prop,
    const reco::BeamSpot& bs, const float vtxTime, const bool matchVertex,
  ...
  for (const DetLayer* ilay : layers) {
    ...
    fillMatchingHits(ilay, tsos, traj, pmag2, pathlength0, trs0, hits, prop, bs,
                     vtxTime, matchVertex, output, bestHit);
  }
...
template <class TrackCollection>
TransientTrackingRecHit::ConstRecHitContainer
TrackExtenderWithMTDT<TrackCollection>::tryETLLayers(
    const TrajectoryStateOnSurface& tsos, const Trajectory& traj,
    const float pmag2, const float pathlength0, const TrackSegments& trs0,
    const MTDTrackingDetSetVector& hits, const MTDDetLayerGeometry* geo,
    const MagneticField* field, const Propagator* prop,
  ...
  for (const DetLayer* ilay : layers) {
    const BoundDisk& disk =
        static_cast<const ForwardDetLayer*>(ilay)->specificSurface();
  ...
  if (output.size() == 2) {
    if (std::abs(output[0]->globalPosition().z()) >
        std::abs(output[1]->globalPosition().z())) {
...
template <class TrackCollection>
void TrackExtenderWithMTDT<TrackCollection>::fillMatchingHits(
    const DetLayer* ilay, const TrajectoryStateOnSurface& tsos,
    const Trajectory& traj, const float pmag2, const float pathlength0,
    const TrackSegments& trs0, const MTDTrackingDetSetVector& hits,
    const Propagator* prop, const reco::BeamSpot& bs, const float& vtxTime,
  ...
  auto find_hits = std::bind(
      find_hits_in_dets, std::cref(hits), std::cref(traj), ilay,
      std::cref(tsos), pmag2, pathlength0, trs0, _1, std::cref(bs),
      bsTimeSpread_, prop, theEstimator.get(), _2, std::ref(hitsInLayer));
  ...
  if (useVertex_ && matchVertex && !hitMatched) {
    ...
    if (!hitsInLayer.empty()) {
      auto const& firstHit = *hitsInLayer.begin();
      LogTrace("TrackExtenderWithMTD")
          << "TrackExtenderWithMTD: matching trial 2: estChi2= "
          << firstHit.estChi2 << " timeChi2= " << firstHit.timeChi2;
  ...
  if (hitMatched) {
    LogTrace("TrackExtenderWithMTD")
        << "TrackExtenderWithMTD: matched hit with time: "
        << bestHit.hit->time() << " +/- " << bestHit.hit->timeError();
  } else {
...
template <class TrackCollection>
reco::Track TrackExtenderWithMTDT<TrackCollection>::buildTrack(
    const reco::TrackRef& orig, const Trajectory& traj,
    const Trajectory& trajWithMtd, const reco::BeamSpot& bs,
    const MagneticField* field, const Propagator* thePropagator, bool hasMTD,
    float& pathLengthOut, float& tmtdOut, float& sigmatmtdOut, float& tofpi,
    float& tofk, float& tofp, float& sigmatofpi, float& sigmatofk,
    float& sigmatofp) const {
  TrajectoryStateClosestToBeamLine tscbl;
  bool tsbcl_status =
      getTrajectoryStateClosestToBeamLine(traj, bs, thePropagator, tscbl);

  if (!tsbcl_status) return reco::Track();
  ...
  auto routput = [&]() {
    return reco::Track(traj.chiSquared(), int(ndof), pos, mom,
                       tscbl.trackStateAtPCA().charge(),
                       tscbl.trackStateAtPCA().curvilinearError(), orig->algo(),
                       reco::TrackBase::undefQuality, t0, betaOut, covt0t0,
                       covbetabeta);
  };

  // compute path length for time backpropagation, using first MTD hit for the
  // momentum
  if (hasMTD) {
    float pathlength;
    TrackSegments trs;
    bool validpropagation =
        trackPathLength(trajWithMtd, bs, thePropagator, pathlength, trs);
    float thit = 0.f;
    float thiterror = -1.f;
    bool validmtd = false;
    ...
    for (auto const& hit : trajWithMtd.measurements()) {
      if (hit.recHit()->geographicalId().det() == DetId::Forward &&
          ForwardSubdetector(hit.recHit()->geographicalId().subdetId()) ==
              FastTime) {
        ihitcount++;
        if (MTDDetId(hit.recHit()->geographicalId()).mtdSubDetector() ==
            MTDDetId::MTDType::ETL) {
          ietlcount++;
        }
      }
    }

    LogTrace("TrackExtenderWithMTD") << "TrackExtenderWithMTD: selected #hits "
                                     << ihitcount << " from ETL " << ietlcount;

    auto ihit1 = trajWithMtd.measurements().cbegin();
    if (ihitcount == 1) {
      const MTDTrackingRecHit* mtdhit =
          static_cast<const MTDTrackingRecHit*>((*ihit1).recHit()->hit());
      thit = mtdhit->time();
      ...
      //
      // The information of the two ETL hits is combined and attributed to the
      // innermost hit
      //
      if (etlpathlength == 0.f) {
        ...
        const MTDTrackingRecHit* mtdhit1 =
            static_cast<const MTDTrackingRecHit*>((*ihit1).recHit()->hit());
        const MTDTrackingRecHit* mtdhit2 =
            static_cast<const MTDTrackingRecHit*>(
                (*(ihit1 + 1)).recHit()->hit());
        TrackTofPidInfo tofInfo = computeTrackTofPidInfo(
            lastStep.second, etlpathlength, trs, mtdhit1->time(),
            mtdhit1->timeError(), 0.f, 0.f, true, TofCalc::kCost);
        //
        // Protect against incompatible times
        //
        float err1 = tofInfo.dterror2;
        float err2 = mtdhit2->timeError() * mtdhit2->timeError();
        if (cms_rounding::roundIfNear0(err1) == 0.f ||
            cms_rounding::roundIfNear0(err2) == 0.f) {
          ...
          if ((tofInfo.dt - mtdhit2->time()) * (tofInfo.dt - mtdhit2->time()) <
            ...
            //
            // Subtract the ETL time of flight from the outermost measurement,
            // and combine it in a weighted average with the innermost the mass
            // ambiguity related uncertainty on the time of flight is added as
            ...
            LogTrace("TrackExtenderWithMTD")
                << "TrackExtenderWithMTD: p trk = " << p.mag()
                << " ETL hits times/errors: " << mtdhit1->time() << " +/- "
                << mtdhit1->timeError() << " , " << mtdhit2->time() << " +/- "
                << mtdhit2->timeError() << " extrapolated time1: " << tofInfo.dt
                << " +/- " << tofInfo.dterror << " average = " << thit
                << " +/- " << thiterror;
            validmtd = true;
          } else {
            // if back extrapolated time of the outermost measurement not
            // compatible with the innermost, keep the one with smallest error
            if (err1 <= err2) {
              thit = tofInfo.dt;
      ...
      edm::LogInfo("TrackExtenderWithMTD")
          << "MTD hits #" << ihitcount << "ETL hits #" << ietlcount
          << " anomalous pattern, skipping...";
    }

    if (validmtd && validpropagation) {
      // here add the PID uncertainty for later use in the 1st step of 4D vtx
      // reconstruction
      TrackTofPidInfo tofInfo = computeTrackTofPidInfo(
          p.mag2(), pathlength, trs, thit, thiterror, 0.f, 0.f, true,
          TofCalc::kSegm, SigmaTofCalc::kCost);
...
template <class TrackCollection>
reco::TrackExtra TrackExtenderWithMTDT<TrackCollection>::buildTrackExtra(
  ...
  bool inside =
      outerDet->surface().bounds().inside(outerDet->toLocal(outerTSOSPos));

  GlobalPoint hitPos = (outerRecHit->isValid())
                           ? outerRecHit->globalPosition()
                           : outerTSOS.globalParameters().position();

  if (!inside) {
    LogTrace(metname)
        << "The Global Muon outerMostMeasurementState is not compatible with "
           "the recHit detector!"
...
template <class TrackCollection>
string TrackExtenderWithMTDT<TrackCollection>::dumpLayer(
