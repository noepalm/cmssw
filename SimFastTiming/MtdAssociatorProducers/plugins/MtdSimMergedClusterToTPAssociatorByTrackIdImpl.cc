...
reco::MergedSimToTPCollectionMtd MtdSimMergedClusterToTPAssociatorByTrackIdImpl::associateSimToTP(
  ...
  // LogDebug("MtdSimMergedClusterToTPAssociator")
  //     << " Found " << simClusters.size() << " MtdSimMergedClusters in the event";
  ...
  //   MtdSimMergedClusterRef simClusterRef = edm::Ref<MtdSimMergedClusterCollection>(simClusH, simClusIndex);
  ...
  //       LogDebug("MtdSimMergedClusterToTPAssociator::associateSimToTP")
  //           << "MtdSimMergedCluster: index = " << simClusIndex << "   simClus TrackId = " << simClusTrackId
...
reco::TPToMergedSimCollectionMtd MtdSimMergedClusterToTPAssociatorByTrackIdImpl::associateTPToSim(
  ...
  // // -- Loop over MtdSimMergedClusters and build a temporary map of trackId, eventId --> simClusterRef
  // std::map<std::pair<unsigned int, uint32_t>, std::vector<MtdSimMergedClusterRef>> simClusIdMap;
  // for (auto simClusIt = simClusters.begin(); simClusIt != simClusters.end(); simClusIt++) {
  //   const auto& simClus = *simClusIt;
  //   EncodedEventId simClusEventId = simClus.eventId();
  //   for (unsigned int igt = 0; igt < simClus.g4Tracks().size(); igt++) {
  //     unsigned int simClusTrackId = simClus.g4Tracks()[igt].trackId();
  //     MtdSimMergedClusterRef simClusterRef =
  //         edm::Ref<MtdSimMergedClusterCollection>(simClusH, simClusIt - simClusters.begin());
  ...
  //         MtdSimMergedClusterRef simClusterRef = simClusIdMap[uniqueId][i];
  ...
  //         LogDebug("MtdSimMergedClusterToTPAssociator")
  //             << " --> Found associated MtdSimMergedCluster:  simClus TrackId = "
