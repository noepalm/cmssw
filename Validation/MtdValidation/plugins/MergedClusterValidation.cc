...
void MergedClusterValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {    
    ...
    // debug
    std::cout << "Valid clusters for event " << evt_event_ << ":" << std::endl;
    for (const auto* cluster : validClusters) {
        std::cout << "  DetId " << cluster->id().rawId() << std::endl;  
    }
    ...
    for (size_t mcIndex = 0; mcIndex < mergedVec.size(); ++mcIndex) {
        ...
        for (const auto& did : mc->clusterIds()) {
                if (did == mc->id()) { seedFound = true; break; }
        }
        if (!seedFound) {
            std::cout << "[WARN] Validation: merged cluster seed " << mc->id().rawId()
                        << " not found among clusterIds() (nClusters=" << mc->nClusters() << ")" << std::endl;
        }

        std::set<uint32_t> ids;
        for (const auto& did : mc->clusterIds()) {
            if (!ids.insert(did.rawId()).second) {
                std::cout << "[WARN] Validation: duplicate detId " << did.rawId()
                        << " in merged cluster seed " << mc->id().rawId() << std::endl;
            }
            if (clusterMap.find(BTLDetId(did)) == clusterMap.end()) {
                std::cout << "[WARN] Validation: clusterId " << did.rawId()
                            << " references reco DetId " << did.rawId() << " not found in input clusters" << std::endl;  
            }
        }
        ...
        if (mc->nClusters() == 1) {
            ...
            if (itVec != clustersByDetId.end() && !itVec->second.empty()) {
                ...
                if (original == nullptr) original = itVec->second.front();
                ...
                std::cout << "[WARN] Validation: single-cluster merged cluster references reco DetId "
                        << mc->clusterIds()[0].rawId() << " not found in input clusters" << std::endl;  
            }
        }
            ...
            if (nComb <= 2000) {
                ...
                std::function<void(size_t)> dfs = [&](size_t idx) {
                    if (idx == candidates.size()) {
                        double s = 0.0;
                        for (const auto* c : current) s += c->energy();
        ...
        if (std::abs(deltaE) > 1e-3f) {
            if (std::abs(deltaE) > 0.5f) {
                std::cout << "[OUTLIER] Merged energy differs from sum(inputs) by " << deltaE
                        << " MeV for seed " << mc->id().rawId() << " nClusters=" << mc->nClusters() << std::endl;
                        std::cout << "  MergedCluster: id=" << mc->id().rawId()
                        << " E=" << mc->energy() << " t=" << mc->time() << " nIds=" << mc->nClusters() << std::endl;
                for (const auto& detId : mc->clusterIds()) {
                    uint32_t dr = detId.rawId();
                    std::cout << "  Constituent DetId: " << dr << std::endl;
                    auto itVec = clustersByDetId.find(dr);
                    if (itVec == clustersByDetId.end() || itVec->second.empty()) {
                        std::cout << "    NO candidates found in reco collection for this DetId\n";
                        continue;
                    }
                    // list all candidates
                    int idx = 0;
                    for (const auto* cand : itVec->second) {
                        std::cout << "    cand[" << idx++ << "] ptr=" << cand
                                << " E=" << cand->energy() << " t=" << cand->time()
                                << " tErr=" << cand->timeError() << std::endl;
                    }
                    // show chosen best (by pickBestMatch used above)
                    const FTLCluster* chosen = pickBestMatch(itVec->second, mc->time());
                    if (chosen) {
                        std::cout << "    chosen ptr=" << chosen 
                                << " E=" << chosen->energy() << " t=" << chosen->time()
                                << " tErr=" << chosen->timeError() << std::endl;
                    } else {
                        std::cout << "    pickBestMatch returned nullptr\n";
                    }
                } // per-DetId loop
                std::cout << "  End debug for merged seed " << mc->id().rawId() << "\n";
            }
        }
        std::cout << "Filling eff histogram: E=" << mc->energy()
        << " T=" << mc->time() << std::endl;
        ...
        //std::cout << "[DEBUG] MergedCluster created with " << mc.nClusters() 
        //  << " clusters, IDs: ";
        ...
        if (cluIds.size() == 2) {
            ...
            for (const auto& detId : cluIds) {
                ...
                if (it != clusterMap.end()) {
                    ...
                    if (thedet) {
                        GlobalPoint global_point = thedet->surface().toGlobal(
                            LocalPoint(cluster->x(), cluster->y(), 0));
                        clusterPositions.push_back(global_point);
                    } else {
                        std::cout << "Warning: no geometry for cluster detid " << cluId.rawId() << std::endl;
                    }
                } else {
                    std::cout << "Warning: cluster detid " << detId.rawId() << " not found in cluster map." << std::endl;
                }
            }
        ...
        std::cout << "detIdToSCIndex contents for event " << evt_event_ << ":" << std::endl;
        for (const auto& entry : detIdToSCIndex) {
            std::cout << "  DetId " << entry.first << " -> MergedCluster " << entry.second << std::endl;
        }
    ...
    for (const auto* cluster : validClusters) {
        if (countedDetIds.count(cluster->id().rawId())) continue;
        ...
        if (!inMergedCluster) {
            //std::cout << "[DEBUG] Filling eff with STANDALONE cluster: E=" << cluster->energy() 
            //      << " MeV, T=" << cluster->time() << " ns" << std::endl;
    ...
    std::cout << "MergedClusters and their constituent DetIds for event " << evt_event_ << ":" << std::endl;    
    for (size_t i = 0; i < mergedVec.size(); ++i) {
        const FTLMergedCluster* mc = mergedVec[i];
        std::cout << "  MergedCluster " << i << ": ";
        for (const auto& detId : mc->clusterIds()) std::cout << detId.rawId() << " ";
        std::cout << std::endl;
    }
    ...
    for (const auto& entry : detIdToSCIndex) {
        if (validDetIds.find(entry.first) == validDetIds.end()) {
            std::cout << "WARNING: DetId " << entry.first << " in detIdToSCIndex but not in validClusters!" << std::endl;
        }
    }
    for (const auto& detId : validDetIds) {
        if (detIdToSCIndex.find(detId) == detIdToSCIndex.end()) {
            std::cout << "WARNING: DetId " << detId << " in validClusters but not in detIdToSCIndex!" << std::endl;
        }
    }
    for (const auto& pair : clustersByDetId) {
        if (pair.second.size() > 1) {
            std::cout << "WARNING: Multiple clusters with DetId " << pair.first << std::endl;
        }
    }
            ...
            for (int etaOffset : etaOffsets) {
                ...
                if (hasOppositeEdgeHit) {
                    ...
                    if (hasMatchingTrackID && !sameIndexMerged) {
                        std::cout << "Not merged but same TrackID clusters: "
                                  << std::endl;

                        if (clusterPtrToSCIndex.count(cluster)) {
                            std::cout << "  Cluster " << cluster->id().rawId() << " is in MergedCluster " << clusterPtrToSCIndex[cluster] << std::endl;
                        } else {
                            std::cout << "  Cluster " << cluster->id().rawId() << " is NOT in any MergedCluster" << std::endl;
                        }
                        if (clusterPtrToSCIndex.count(adjCluster)) {
                            std::cout << "  Cluster " << adjCluster->id().rawId() << " is in MergedCluster " << clusterPtrToSCIndex[adjCluster] << std::endl;
                        } else {
                            std::cout << "  Cluster " << adjCluster->id().rawId() << " is NOT in any MergedCluster" << std::endl;
                        }
                        std::cout << "  Cluster 1 edge hits: ";
                        for (int i = 0; i < cluster->size(); ++i) {
                            auto hit = cluster->hit(i);
                            if (hit.y() == 0 || hit.y() == 15) {
                                std::cout << hit.y() << " ";
                        ...
                        std::cout << "  Cluster 1 time: " << cluster->time() << " ± " << cluster->timeError() << std::endl;
                        std::cout << "  Cluster 2 time: " << adjCluster->time() << " ± " << adjCluster->timeError() << std::endl;
                        ...
                        std::cout << "  Δt = " << dt << ", threshold = " << (10 * combinedError) << std::endl;

                        std::cout << "  Cluster 1 ieta, iphi: " << ieta << ", " << iphi << std::endl;
                        auto adjIndices = topology->btlIndex(adjDetId.rawId());
                        std::cout << "  Cluster 2 ieta, iphi: " << adjIndices.second << ", " << adjIndices.first << std::endl;

                        std::cout << "  Cluster 1 energy: " << cluster->energy() << std::endl;
                        std::cout << "  Cluster 2 energy: " << adjCluster->energy() << std::endl;
                    }
    ...
    std::cout << "Event " << evt_event_ << ": " << mc_n_ << " MergedClusters, " 
        ...
        std::cout << "Same TrackID pairs: " << sameTrackIDPairs
                  << ", Merged same TrackID pairs: " << mergedSameTrackIDPairs << std::endl;

    if (h_eta_adjacent_pairs_->getTH1F()->GetEntries() > 0) {
        h_eta_merging_fraction_->getTH1F()->Divide(
            h_eta_merged_pairs_->getTH1F(), 
            h_eta_adjacent_pairs_->getTH1F(), 
            1, 1, "B");
    }
    
    if (h_eta_sameTrackID_pairs_->getTH1F()->GetEntries() > 0) {
        h_merging_efficiency_->getTH1F()->Divide(
            h_eta_merged_sameTrackID_pairs_->getTH1F(), 
            h_eta_sameTrackID_pairs_->getTH1F(), 
            1, 1, "B");
    }
    ...
    TH1F* h_time_ind = h_cluster_time_->getTH1F();
    TH1F* h_time_eff = h_cluster_time_eff_->getTH1F();
...
void MergedClusterValidation::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const&, edm::EventSetup const&) {
    ...
    h_mc_cluster_distance_phi_ = ibooker.book1D("h_mc_cluster_distance_phi", "Distance in Phi Between Clusters in MergedCluster;#Delta'phi;Entries", 50, -0.1, 0.1);
    h_mc_cluster_distance_eta_ = ibooker.book1D("h_mc_cluster_distance_eta", "Distance in Eta Between Clusters in MergedCluster;#Delta'eta;Entries", 50, -0.1, 0.1);
    h_mc_cluster_distance_z_ = ibooker.book1D("h_mc_cluster_distance_z", "Distance in Z Between Clusters in MergedCluster;#Delta'z [mm];Entries", 100, -50., 50.);
    h_mc_cluster_distance_2D_ = ibooker.book2D("h_mc_cluster_distance_2D", "Distance in Eta vs Phi Between Clusters in MergedCluster;#Delta'phi;#Delta'z", 50, -0.1, 0.1, 100, -50., 50.);
    
    h_mc_cluster_distance_ieta_ = ibooker.book1D("h_mc_cluster_distance_ieta", "Distance in iEta Between Clusters in MergedCluster;#Delta iEta;Entries", 10, -5, 5);
    h_mc_cluster_distance_iphi_ = ibooker.book1D("h_mc_cluster_distance_iphi", "Distance in iPhi Between Clusters in MergedCluster;#Delta iPhi;Entries", 10, -5, 5);
    h_mc_cluster_distance_i2D_ = ibooker.book2D("h_mc_cluster_distance_i2D", "Distance in iEta vs iPhi Between Clusters in MergedCluster;#Delta iPhi;#Delta iEta", 11, -5, 5, 11, -5, 5);
    ...
    h_eta_merging_fraction_ = ibooker.book1D("h_eta_merging_fraction", "Merging Fraction vs Eta;#eta;Fraction", 100, -1.5, 1.5);
    
    h_eta_sameTrackID_pairs_ = ibooker.book1D("h_eta_sameTrackID_pairs", "Pairs with Same TrackID vs Eta;#eta;Number of Pairs", 100, -1.5, 1.5);
    h_eta_merged_sameTrackID_pairs_ = ibooker.book1D("h_eta_merged_sameTrackID_pairs", "Merged Pairs with Same TrackID vs Eta;#eta;Number of Pairs", 100, -1.5, 1.5);
    h_merging_efficiency_ = ibooker.book1D("h_merging_efficiency", "Merging Efficiency vs Eta;#eta;Efficiency", 100, -1.5, 1.5);
    ...
    for (int phi_idx = 0; phi_idx < 6; phi_idx++) {
        for (int eta_idx = 0; eta_idx < 6; eta_idx++) {
            ...
            std::string timeResName = "BtlTimeResPhi" + std::to_string(static_cast<int>(phi_min * 10)) + "to" + std::to_string(static_cast<int>(phi_max * 10)) + "Eta" + std::to_string(static_cast<int>(eta_min * 10)) + "to" + std::to_string(static_cast<int>(eta_max * 10));
    ...
    h_single_dt_ = ibooker.book1D("h_single_dt", "Merged - Cluster Time (single); #Delta Time [ns]; Entries", 50, -.5, .5);
    h_single_de_ = ibooker.book1D("h_single_de", "Merged - Cluster Energy (single); Energy Ratio; Entries", 50, -0.1, .1);
    h_single_dt_outlier_ = ibooker.book1D("h_single_dt_outlier", "Outlier Δt (single);#Delta t [ns];Entries", 50, -10, 10);
    h_single_de_outlier_ = ibooker.book1D("h_single_de_outlier", "Outlier ΔE (single);#Delta E [MeV];Entries", 50, -50, 50);
    // energy consistency: merged - sum(inputs)
    h_mc_energy_minus_sumInputs_ = ibooker.book1D("h_mc_energy_minus_sumInputs", "Merged E - sum(input E);ΔE [MeV];Entries", 100, -1.0, 1.0);
